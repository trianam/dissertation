...
\imageDouble{scrSites-a.png}{scrSites-b.png}{Scene with Voronoi sites (on
  obstacles on the left and on obstacles and bounding box on the right).}{fig:sites}
...
collides with another triangle $T_2=\triangle
\ve{a_2}\ve{b_2}\ve{c_2}$. Furthermore, when a
collision is found, is possible to 
obtain also the barycentric coordinates on
$\ve{a_2},\ve{b_2},\ve{c_2}$ of the points where the 
edges of $T_1$ intersect $T_2$ (or vice versa the coordinates on
$\ve{a_1},\ve{b_1},\ve{c_1}$ of the points where $T_2$ intersect $T_1$).
...
after extracting the path\footnote{If the user chooses to minimize the
...
constraint(P) = \frac{\left|\ \{\ve{p} \in spline(P)\ : \ve{p}\in
    ...
    spline(P)\}\ \right|}
\end{equation}
where the points $\ve{p}$ of the spline are calculated in a discrete
form. Thus the constraint depends on the tabulation of the curve and
it is also possible to have borderline cases where the calculated
constraint doesn't reflect the real situation. The
