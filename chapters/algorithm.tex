...
in $P$ intersect one of the \ac{OTF}. 
...
\subsubsection{Complexity considerations}\index{Post process!complexity}
...
i.e. $|\ve{v_1}-\ve{v_0}|=|\ve{v_2}-\ve{v_1}|=\cdots=|\ve{v_n}-\ve{v_{n-1}}|$
- then we agree on a uniform partition of the knots, as in
\cref{fig:adaptive1} where
$t_{m+1}-t_m=t_{m+2}-t_{m+1}=\cdots=t_{n+1}-t_n$. Starting from this situation if
...
respect to the optimal case, where $\nu_j$ is the projection of
$\ve{v_j}$ on the axis of the parameter where $\ve{v_0}$ correspond to
$a=0$, $\ve{v_n}$ 
correspond to $b=1$ and the scale of the lengths are preserved.
