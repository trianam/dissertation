...
\begin{algorithm}
 ...
 \begin{algorithmic}[1]
   ...
   \EndFor
   \EndFor
   \ForAll{$o\in rightOut$}\label{ln:tripleFor1}
   \ForAll{$i\in leftIn$}\label{ln:tripleFor2}
   \State $E_t = E_t\cup (o,i)$
   \EndFor
   \EndFor
   \EndFor
...
if a triangle $T_1=\triangle A_1B_1C_1$
collides with another triangle $T_2=\triangle A_2B_2C_2$ and, when a
collision is found, how to 
obtain also the barycentric coordinates on $A_2,B_2,C_2$ of the points
where the 
edges of $T_1$ intersect $T_2$ (or the coordinates on $A_1,B_1,C_1$ of
...
that set $O$.
\begin{myfig}{$T_i$($=\triangle V_iV_{i+1}V_{i+2}$) and the points
    $O_1,O_2,O_3$ of intersection between it and the edges of some \ac{OTF}.}{fig:triangleIntersection}
  \begin{tikzpicture}
    \coordinate (P) at (-1,0);
    \coordinate (A) at (0,0);
    \coordinate (B) at (4,5);
    ...
    \node[below right=0.2em] (O1n) at (O1) {$O_1$};
    \node[below=0.2em] at (O1n) {$\scriptscriptstyle(\equiv O_{near})$};
