...
\section{B-splines curves properties}\label{sec:bsplineProp}
In this section we describe some properties of B-spline curves that we
used for the development of the project.

\subsection{Convex hull}
A B-spline curve $\ve{S}(t)$ of order $m$ defined by the control polygon
$\ve{v_0},\ve{v_1},\dots,\ve{v_n}$ is contained inside the union of the
convex hulls composed of $m+1$ vertices of the control polygon. If we
...
\begin{myfig}{Convex hull containing B-spline of grade 2}{fig:convexHull}
  \begin{tikzpicture}
    \coordinate (a) at (0,0);
...
and the surface where a quadratic B-spline applied to
it can be contained. Note that the convex hull property holds also in
3-dimensional
space - i.e. a quadratic B-spline in 3-dimensional space is contained
